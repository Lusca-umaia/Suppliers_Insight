      ...
      <div className="mx-auto max-w-7xl overflow-hidden px-6 pt-24 pb-6 sm:pt-32 lg:px-8">
        ...
        <div className="mt-10 flex justify-center space-x-10">
          {socialMedia.map((item) => (
            <a key={item.href} href={item.href} target='_blank' className="text-gray-400 hover:text-gray-500">
              <item.icon className="h-6 w-6" />
            </a>
          ))}
        </div>
        <p className="mt-10 text-center text-xs leading-5 text-gray-500">
          &copy; Copyright &copy; 2024 Lucas Maia – Todos os direitos reservados.
        </p>
      </div>
    </footer>
  )
