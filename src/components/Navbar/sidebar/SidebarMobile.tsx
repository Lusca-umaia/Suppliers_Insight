...
export default function SidebarMobile(props: IProps) {
  ...
  return (
    <>
      {props.openMenu && <div onClick={() => props.setOpenMenu()} className="lg:hidden fixed inset-0 backdrop-blur-md z-0 animate-appearance"></div>}
      < div className={
        classNames(
        ...
        <ul className={"flex flex-col max-[340px]:w-full w-72 gap-2.5 duration-100 h-full bg-white py-2.5 border-r px-2 shadow-inner"}>
          <button
            type="button"
          ...
          {navigationOptions.map((option, index) => (
            <li
              ...
              <span className={"w-full text-sm font-semibold duration-100"}>{option.name}</span>
            </li>
          ))}
        </ul>
      </div >
